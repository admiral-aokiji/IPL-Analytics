...
drop table if EXISTS deliveries2;
CREATE TABLE if not exists deliveries2 (
...
into table deliveries2 
fields terminated by ',' 
enclosed by '"' 
lines terminated by '\n' 
ignore 1 lines;

update deliveries2 set player_dismissed = null where player_dismissed = '';
...
BEGIN 
    ...
    create temporary table wicket select match_id,inning, bowler, `over` overs, count(case when player_dismissed is null then null else 1 end ) wickets FROM deliveries2 d WHERE d.match_id between startMatchID and endMatchID group by match_id,inning, `over`;
...
select match_id, inning, bowler, sum(balls) as balls,sum(maiden) maidens, (sum(near_maiden) - sum(maiden)) near_maidens, sum(runs) runs_conceded, sum(wickets) wickets, sum(dot_balls) dots, sum(singles) singles, sum(doubles) doubles, sum(triples) triples, sum(fours) fours, sum(sixes) sixes, sum(extras) - sum(wide_runs) - sum(noball_runs) bat_extras, sum(wide_runs) wide_runs, sum(noball_runs) noball_runs from (select match_id, inning, bowler, over_no, balls, runs, wickets, extras, dot_balls, singles, doubles, triples, fours, sixes, wide_runs, noball_runs, (runs = legbye_runs + bye_runs and balls = 6) maiden, (dot_balls > 4) near_maiden from match_over_scorecards) as t group by match_id, inning, bowler; 
...
BEGIN 
	...
	select match_id, inning, batsman, sum(batsman_runs) runs, count(case when noball_runs = 0 and (extras_type is null or extras_type not like 'wide%') then 1 else null end) balls, count(case when total_runs = 0 or (extras_type = 'byes' and noball_runs = 0) or (extras_type = 'legbyes' and noball_runs = 0) then 1 else null end) dots, count(case when batsman_runs = 1 then 1 else null end) singles, count(case when batsman_runs = 2 then 1 else null end) doubles, count(case when batsman_runs = 3 then 1 else null end) triples, count(case when batsman_runs = 4 then 1 else null end) fours, count(case when batsman_runs = 5 then 1 else null end) fives, count(case when batsman_runs = 6 then 1 else null end) sixes from deliveries2 where (match_id between startMatch and endMatch) group by match_id,inning, batsman;
    -- Done till here
    create TEMPORARY table non_strike_min
    select match_id, inning, non_striker, min(team_overs) as str_min, min(team_runs) as min_truns, 2 as pos from deliveries2 where (match_id between startMatch and endMatch) GROUP BY match_id, inning, non_striker;
    create TEMPORARY table strike_min
    select match_id, inning, batsman, min(team_overs) as str_min, min(team_runs) as min_truns, 1 as pos from deliveries2 where (match_id between startMatch and endMatch) GROUP BY match_id, inning, batsman; 
	...
	select match_id, inning, batsman, dismissal_kind, bowler, fielder, player_dismissed, non_striker_runs as end_partner_runs, non_striker_balls as end_partner_balls, team_runs, team_overs from deliveries2 where player_dismissed is not null and player_dismissed = batsman and (match_id between startMatch and endMatch) union
...
-- PYTHON SCRIPT FOR INSERTING MATCH-DAY SQUADS
